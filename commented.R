...
# Let's use the caffeine experiment data that we cleaned in the cleaning data
# script. Hopefully you saved the cleaned version.
...
# control_index <- which(dat$condition == "control")
...
# round(x = table(dat$ethnicity) / n, digits = 2)
...
# the cross-classification of mulitiple categorical variables. This is also
...
# age_by_cond_form <- age ~ condition # We'll see these more later.
...
# don't really have a good reason to beleive that these differences will affect
# math problem response times or the effects of caffeine.

# Let's look at our outcome variables, response times.
...
# rt_index <- grep("rt_", colnames(dat))
...
# Columns means
# apply(X = dat[,rt_index], MARGIN = 2, FUN = mean)
...
# apply(X = dat[,rt_index], MARGIN = 2, FUN = mean, na.rm = T)

# same as
# colMeans(dat[,rt_index], na.rm = T)
